...
export function countBarGraph() {
  ...
  let i = 0;
  let count = 0;
  while (i < $(element).$$('XCUIElementTypeOther').length) {
...
export function clickOnFirstAreasInList() {
  HealthMapsScreen.scrollDownToElement(HealthMapsScreen.firstAreaInList, 20);
  HealthMapsScreen.firstAreaInList.click();
}

export function clickOnFirstDistrictInList() {
  HealthMapsScreen.scrollDownToElement(
    HealthMapsScreen.firstDistrictInList,
    20
  );
  HealthMapsScreen.firstDistrictInList.click();
}
...
export function typeClinicToSearch(areas) {
  ...
  HealthMapsScreen.waitForElementIsShown(
    HealthMapsScreen.searchClinicTextField
  );
  HealthMapsScreen.searchClinicTextField.setValue(areas);
  tapOnSearchButton();
}
...
export function isClinicsFound() {
  return HealthMapsScreen.numberClinicFound.isExisting();
}

export function isNoClinicAvailableDisplayed() {
  return HealthMapsScreen.noClinicAvailable.isExisting();
}
...
export function clickOnDirectionButton() {
  HealthMapsScreen.waitForElementIsShown(HealthMapsScreen.directionsButton);
  HealthMapsScreen.directionsButton.click();
}

export function isDirectionPageDisplayed() {
  const platform = getPlatform();
  if (platform === 'ios') {
    return $('~Change Route').isExisting();
  } else {
    return $(txt('Make it your map')).isExisting(); // Notice: Base on each google map app on each device
  }
}
export function isCallPageDisplayed() {
  ...
  if (platform === 'ios') {
    ...
    return $(txt('Create new contact')).isExisting();
  }
}
export function clickOnCallButton() {
  HealthMapsScreen.waitForElementIsShown(HealthMapsScreen.callButton);
  HealthMapsScreen.callButton.click();
}
...
export function searchForClinic(clinic) {
  ...
  HealthMapsScreen.searchClinicTextField.click();
  HealthMapsScreen.searchClinicTextField.setValue(clinic);
  tapOnSearchButton();
}
...
export function clickOnDirectionsButtonOnDetailsPage(directions) {
  ...
  if (platform === 'ios') {
    element = $(
      `//XCUIElementTypeOther[@name="Rejected ${directions} Right Arrow Icon"]`
    );
  } else {
...
export function clickOnCallButtonOnDetailsPage(number) {
  ...
  if (platform === 'ios') {
    element = $(
      `//XCUIElementTypeOther[@name="Rejected ${number} Right Arrow Icon"]`
    );
  } else {
    element = $('~Phone Icon');
  }
...
export function isClinicDisplayeWithName() {
  let element = $(
    `//XCUIElementTypeOther[@name="Compass My location Google Maps”]/../XCUIElementTypeOther[2]/XCUIElementTypeOther`
  );
  HealthMapsScreen.waitForElementIsShown(element);
...
export function isClinicDisplayed() {
  ...
  if (platform === 'ios') {
    ...
    let element = $(`//android.support.v4.view.ViewPager`);
    //let element = $(`//android.support.v4.view.ViewPager/android.view.ViewGroup/android.view.ViewGroup[2]/android.view.ViewGroup[1]`);
    HealthMapsScreen.waitForElementIsShown(element);
    return element.isExisting();
  }
}
...
export function isBarGraphRiskDisplay() {
  HealthScreen.scrollDownToElement(HealthScreen.historyGraph, 10);
  return HealthScreen.historyGraph.isExisting();
}
...
export function clickSearchForClinicButton() {
  HealthScreen.waitForIsShown(true);
  HealthScreen.scrollDownToElement(HealthScreen.searchForClinicButton, 50);
  HealthScreen.searchForClinicButton.click();
}
...
export function denyAccessLocation() {
  ...
  if (platForm === 'android') {
    if ($txt('DENY', 'android.widget.Button').isExisting()) {
      $txt('DENY', 'android.widget.Button').click();
      $txt('DENY', 'android.widget.Button').click();
    }
  }
}
