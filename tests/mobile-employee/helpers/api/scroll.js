import { swipeUp, swipeDown, swipeLeft } from './swipe';
...
export function swipeOnElementToLeft(element, maxScrolls, amount = 0) {
  if (
    (!element.isExisting() || !element.isDisplayed()) &&
    amount <= maxScrolls
  ) {
    swipeLeft(1.1);
    swipeOnElementToLeft(element, maxScrolls, amount + 1);
  } else if (amount > maxScrolls) {
    throw new Error(
      `The element '${element}' could not be found or is not visible.`
    );
